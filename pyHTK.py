...
def txt2label(fileTxt, fileLab):
	"""
	Convert an orthographycal transcription to the HTK label.  
	:param path fileTxt: path to the text file in which the orthographycal transcription of the utterance is written in one line.
	:param path fileLab: path to the text file in which the contents of fileTxt is written as a word per line. 
	"""
...
def lab2HTKdic(fileLab, fileHTKdic, lex, connect):
	"""
...
def doHVite(fileWav, fileLab, fileHTKdic, fileFA, configHVite, filePhoneList, AcousticModel):
	"""
...
def ForcedAlignment(fileWav, fileTxt, fileOut, configFile, saveIntermediateFiles):
	"""
	Forced Alignment
	:param path fileWav: path to the wav file in which the utterance was recorded.
	:param path fileTxt: path to the text file in which the orthographycal transcription of the utterance is written in one line.
	...
	fileLab = fileWav.replace('.wav', '.lab')
	...
	txt2label(fileTxt, fileLab)
	
	# for each words in the label file pronunciation(s) are searched in lexicon database...
	lab2HTKdic(fileLab, fileHTKdic, lex, connect)

	# forced alignment using HVite
	doHVite(fileWav, fileLab, fileHTKdic, fileFA.name, configHVite, filePhoneList, AcousticModel)
